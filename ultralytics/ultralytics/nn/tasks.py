...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        if self.rgbd_enabled:
            depth_channels = self.rgbd_cfg.get("depth_channels", 1)
            encoder_channels = tuple(self.rgbd_cfg.get("encoder_channels", (32, 64, 128, 256)))
            self.depth_encoder = DepthEncoder(depth_channels, encoder_channels)
            fusion_layers = self.rgbd_cfg.get("fusion_layers", {"p3": 16, "p4": 19, "p5": 22})
            for level, layer_idx in fusion_layers.items():
                if layer_idx >= len(self.model):
                    LOGGER.warning(f"RGBD fusion layer index {layer_idx} exceeds model length, skipping.")
                    continue
                rgb_channels = self._infer_module_channels(self.model[layer_idx])
                depth_c = self.depth_encoder.out_channels.get(level, encoder_channels[-1])
                self.depth_fusions[level] = DepthGuidedFusion(rgb_channels, depth_c)
                self.rgbd_layer_map[layer_idx] = level
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        """自定义前向过程，支持在指定层注入深度引导特征。"""
        depth = None
        if isinstance(x, (tuple, list)):
            depth = x[1] if len(x) > 1 else None
            x = x[0]
        depth_feats = {}
        if self.rgbd_enabled and depth is not None:
        ...
        for m in self.model:
            ...
            if (
                ...
                and (fusion_key := self.rgbd_layer_map.get(m.i))
            ):
                depth_tensor = depth_feats.get(fusion_key)
                x = self.depth_fusions[fusion_key](x, depth_tensor)
    ...
    @staticmethod
    def _infer_module_channels(module: torch.nn.Module) -> int:
        """根据模块的 out_channels 属性推断输出通道数，若缺失则回退到常规推断。"""
        if hasattr(module, "out_channels"):
            return int(module.out_channels)
        for attr in ("cv2", "cv3", "conv"):
        ...
        raise AttributeError(f"无法从模块 {module.__class__.__name__} 推断通道数")
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        m_.out_channels = c2  # expose output channels for downstream customization
        if verbose:
