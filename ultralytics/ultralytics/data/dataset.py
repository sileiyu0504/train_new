...
class YOLODataset(BaseDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        default_replace = ("_color_", "_depth_")
    ...
    def _build_depth_index(self) -> None:
        """构建图像与深度图之间的索引映射，缺失深度时自动填充空值。"""
        if not self.depth_enabled:
            self.depth_files = [None] * self.ni
            return
        depth_dir = self.depth_dir
        if not depth_dir.exists():
            LOGGER.warning(f"{self.prefix}Depth directory {depth_dir} not found, fallback to zero depth.")
            self.depth_files = [None] * self.ni
            self.depth_enabled = False
            return
        depth_paths = self.get_img_files(str(depth_dir))
        lookup = {Path(p).stem: p for p in depth_paths}
        replace_from, replace_to = (self.depth_replace + ("", ""))[:2]
        depth_list: list[str | None] = []
        missing = 0
        for im_file in self.im_files:
            stem = Path(im_file).stem
            candidates = [stem]
            if replace_from and replace_to and replace_from in stem:
                candidates.append(stem.replace(replace_from, replace_to))
            depth_file = next((lookup.get(name) for name in candidates if lookup.get(name)), None)
            if depth_file is None:
                missing += 1
            depth_list.append(depth_file)
        if missing:
            LOGGER.warning(f"{self.prefix}{missing} depth files missing, zero maps will be used instead.")
        self.depth_files = depth_list

    def _load_depth_map(self, index: int, target_shape: tuple[int, int]) -> np.ndarray | None:
        """读取并缩放指定样本的深度图，返回归一化后的单通道图像。"""
        depth_path = self.depth_files[index] if hasattr(self, "depth_files") else None
    ...
    def get_image_and_label(self, index: int) -> dict[str, Any]:
        """扩展父类方法，将深度图像拼接到 RGB 通道末尾供后续变换共享同一几何操作。"""
        ...
        if depth is not None:
            depth_channel = np.clip(depth * 255.0, 0.0, 255.0).astype(img.dtype)
        else:
            depth_channel = np.full((h, w, 1), self.depth_fill_byte, dtype=img.dtype)
        if depth_channel.ndim == 2:
            depth_channel = depth_channel[..., None]
        if depth_channel.shape[-1] != 1:
