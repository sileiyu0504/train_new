...
class RandomHSV:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        img = labels["img"]
        if img.shape[-1] < 3:  # 只有 RGB 通道时才做 HSV 变换
            return labels
        rgb = np.ascontiguousarray(img[..., :3])
        if self.hgain or self.sgain or self.vgain:
            dtype = rgb.dtype  # uint8
...
class Format:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        img = labels.pop("img")
        use_depth = labels.pop("use_depth", False)
        depth = None
    ...
    def _format_depth(self, depth: np.ndarray) -> torch.Tensor:
        """将单通道深度图转换为 PyTorch 张量。"""
